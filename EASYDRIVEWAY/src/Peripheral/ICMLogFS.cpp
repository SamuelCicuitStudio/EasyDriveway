...
void ICMLogFS::loadSDPinsFromConfig() {
    ...
    if (_cfg) {
        ...
        _sdCS    = _cfg->GetInt   (CFG_SD_CS_PIN_KEY,   PIN_SD_CS_DEFAULT);
        _sdSCK   = _cfg->GetInt   (CFG_SD_SCK_PIN_KEY,  PIN_SD_SCK_DEFAULT);
        _sdMISO  = _cfg->GetInt   (CFG_SD_MISO_PIN_KEY, PIN_SD_MISO_DEFAULT);
        _sdMOSI  = _cfg->GetInt   (CFG_SD_MOSI_PIN_KEY, PIN_SD_MOSI_DEFAULT);
    } else {
