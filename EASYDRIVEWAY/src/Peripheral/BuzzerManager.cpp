...
bool BuzzerManager::begin() {
  ...
  _pin        = _cfg->GetInt (NVS_PIN_BUZ_GPIO,         PIN_BUZ_GPIO_DEFAULT);
  _activeHigh = _cfg->GetBool(NVS_PIN_BUZ_AH, BUZ_AH_DEFAULT);
  _enabled    = _cfg->GetBool(NVS_PIN_BUZ_FBK,     BUZ_FBK_DEFAULT);
...
void BuzzerManager::setEnabled(bool en) {
  _enabled = en;
  if (_cfg) _cfg->PutBool(NVS_PIN_BUZ_FBK, en);
  if (!en) stop();
}
