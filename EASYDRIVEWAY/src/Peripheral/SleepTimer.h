...
// Use RTClib types everywhere to avoid duplicate type stubs across TUs.
#include <RTClib.h>
...
public:
    ...
    // Construct with managers (required). On non-ICM, rtc may be nullptr and is ignored.
    ...
    // Initialize: ICM reads RTC INT pin from ConfigManager; others seed timers.
    ...
    // Query
    uint32_t nowEpoch() const;                       // current epoch from chosen time base
...
#ifdef NVS_ROLE_ICM
    // DS3231 alarm helpers via RTClib (ICM only)
...
private:
    RTC_DS3231*     _rtc  = nullptr;     // On non-ICM, unused (may be null)
    ...
    int             _pinRTCInt         = PIN_RTC_INT_DEFAULT;
#endif
