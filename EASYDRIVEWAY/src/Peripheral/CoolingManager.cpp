...
bool CoolingManager::begin() {
    ...
    // Pins & options from config (unified keys)
    _pinFanPwm  = _cfg->GetInt(NVS_PIN_FAN_PWM, PIN_FAN_PWM_DEFAULT);

#if defined(NVS_ROLE_SENS)
    if (!_bme) { logSensorFault("bme_ptr_null"); return false; }
#else
    _pinTemp    = _cfg->GetInt(NVS_PIN_TS_GPIO, PIN_TS_GPIO_DEFAULT);
...
void CoolingManager::periodicUpdate() {
    ...
    float tC = NAN;
#if defined(NVS_ROLE_SENS)
    // Sensor role: read from BME provider (also fetch RH & P)
    float rh = NAN, pPa = NAN;
    if (_bme && _bme->read(tC, rh, pPa)) {
        _lastRH = rh; _lastP = pPa;
    } else {
        if (!isnan(_lastTempC)) { tC = _lastTempC; } else { logSensorFault("bme_read_fail"); }
    }
#else
    // Non-sensor roles: DS18B20 on OneWire
    if (_sensors) {
        _sensors->requestTemperatures();
        tC = _sensors->getTempCByIndex(0);
        ...
        if (tC == DEVICE_DISCONNECTED_C || tC < -55.0f || tC > 125.0f || fabsf(tC - 85.0f) < 0.01f) {
            if (!isnan(_lastTempC)) { tC = _lastTempC; } else { tC = NAN; logSensorFault("temp_invalid"); }
        }
    } else {
        logSensorFault("ds18b20_null");
    }
#endif
...
void CoolingManager::logInit() {
    ...
    ICM_PWR_INFO((*_log), 1200,
        "Cooling init (SENS) pwmPin=%d eco=%.1f norm=%.1f force=%.1f hyst=%.1f eco%%=%u norm%%=%u force%%=%u",
        _pinFanPwm,
        _ecoOnC, _normOnC, _forceOnC, _hystC,
        _ecoPct, _normPct, _forcePct);
#else
