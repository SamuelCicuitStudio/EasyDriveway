...
// ------------------------------------------------------------------
// Role sanity checks (exactly one must be defined BEFORE this header)
// ------------------------------------------------------------------
...
// ==== RTOS task config (can be overridden in Config) ====
...
// Forward-declare BME provider so constructor stays unified for all roles
class BME280Manager;
...
public:
    ...
    // Unified constructor for all roles. For non-sensor roles, `bme` can be nullptr.
    CoolingManager(ConfigManager* cfg, BME280Manager* bme = nullptr, ICMLogFS* log = nullptr)
    ...
    // Runtime info (always available; for non-BME roles RH/P return NaN)
    ...
    void attachLogger(ICMLogFS* log)    { _log = log; }
...
private:
    ...
    // Role-specific sensor setup / read helpers
    void setupSensor();          // DS18B20/other roles
    bool readSensor(float& tC);  // fills tC; role-specific implementation
...
private:
    ...
    BME280Manager*   _bme  = nullptr;   // used only for NVS_ROLE_SENS
    ICMLogFS*        _log  = nullptr;

    // Pins / HW
    int _pinFanPwm = PIN_FAN_PWM_DEFAULT;
    #if !defined(NVS_ROLE_SENS)
    // DS18B20-only settings (ignored for sensor role)
    int  _pinTemp        = PIN_TS_GPIO_DEFAULT;
    bool _tempPullup     = TEMP_SENSOR_PULLUP_DEFAULT;
    #endif

    // DS18B20 handles
    class OneWire*           _oneWire = nullptr;
    class DallasTemperature* _sensors = nullptr;
    ...
    float   _lastRH       = NAN;  // only valid on sensor role
    float   _lastP        = NAN;  // only valid on sensor role
