...
#include "EspNow/ICM_Nw.h"   // setChannel(), serializePeers(), pair(), configureTopology(), exportConfiguration()
...
public: // ======================= LIFECYCLE & CORE =======================
  ...
  void attachLog(ICMLogFS*      Log){_log = Log;};
  void attachNow(NwCore::Core*  Nw){_esn = Nw;};
  void attachSlp(SleepTimer*  slP){_slp = slP;};
