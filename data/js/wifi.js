(() => {
  ...
  // Form elements (match wifi.html IDs)
  const staSsid = $("sta_ssid"),
    staPass = $("sta_pass");
  const apSsid = $("ap_ssid"),
    apPass = $("ap_pass");
  const esnCh = $("espnow_channel");
  const bleName = $("ble_name"),
    blePin = $("ble_pin");
  ...
  async function loadStatus() {
    try {
      ...
      setText(
        netMode,
        mode,
        mode === "STA" ? "ok" : mode === "AP" ? "info" : "danger"
      );
      setBadge(
        netBadge,
        mode === "STA" ? "ok" : mode === "AP" ? "info" : "danger"
      ...
      setText(
        netRssi,
        typeof j.rssi === "number" ? j.rssi + " dBm" : "—",
        typeof j.rssi === "number" ? "info" : "muted"
      );
  ...
  async function loadConfig() {
    try {
      ...
      if (typeof j.ble_password === "number")
        blePin.value = String(j.ble_password).padStart(6, "0");
      else if (typeof j.ble_password === "string")
        blePin.value = j.ble_password;
    } catch (e) {
      console.error(e);
      toastMsg("Load failed");
    }
  }
  ...
  function validate() {
    ...
    if (!(ch >= 1 && ch <= 13)) {
      esnCh.focus();
      throw new Error("Channel must be 1..13");
    }
    if (apPass.value && apPass.value.length > 0 && apPass.value.length < 8) {
      apPass.focus();
      throw new Error("AP password must be ≥ 8 chars");
    }
    if (blePin.value && !/^\d{6}$/.test(blePin.value)) {
      blePin.focus();
      throw new Error("BLE PIN must be 6 digits");
    }
    return { ch };
  }
  ...
  async function doFactoryReset() {
    ...
    try {
      ...
      toastMsg("Factory reset OK");
      setTimeout(loadConfig, 800);
    } catch (e) {
      toastMsg("Factory reset failed");
    }
  }
