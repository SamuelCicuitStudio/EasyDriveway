...
/**
 * @brief Initializes all default keys and variables for the ICM device.
 *
 * Populates Preferences/NVS with sane defaults for identity, wireless,
 * timestamps, and full hardware pin assignment (ICM board).
 */
void ConfigManager::initializeVariables() {
    ...
    // ---------------- Hardware pin assignments (ICM) ------------
    // SD card (SPI)
    PutString(SD_CARD_MODEL_KEY, SD_CARD_MODEL_DEFAULT); // "MKDV8GIL-AST"
    ...
    // External UART to Power Module
    PutInt(PWR_UART_RX_PIN_KEY,  PWR_UART_RX_PIN_DEFAULT);  // 16
    PutInt(PWR_UART_TX_PIN_KEY,  PWR_UART_TX_PIN_DEFAULT);  // 17
    PutInt(PWR_UART_BAUD_KEY,    PWR_UART_BAUD_DEFAULT);    // 115200

    // Fan & RGB status LEDs
    PutInt(FAN_PWM_PIN_KEY,      FAN_PWM_PIN_DEFAULT);      // 8
    PutInt(LED_R_PIN_KEY,        LED_R_PIN_DEFAULT);        // 5
    PutInt(LED_G_PIN_KEY,        LED_G_PIN_DEFAULT);        // 6
    PutInt(LED_B_PIN_KEY,        LED_B_PIN_DEFAULT);        // 7

    // RTC (DS3231MZ+TRL on I2C)
    PutString(RTC_MODEL_KEY,     RTC_MODEL_DEFAULT);        // "DS3231MZ+TRL"
    PutInt   (RTC_INT_PIN_KEY,   RTC_INT_PIN_DEFAULT);      // 19
    PutInt   (RTC_32K_PIN_KEY,   RTC_32K_PIN_DEFAULT);      // 20
    PutInt   (I2C_SCL_PIN_KEY,   I2C_SCL_PIN_DEFAULT);      // 4
